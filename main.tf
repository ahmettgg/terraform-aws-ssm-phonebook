...
resource "aws_launch_template" "asg-lt" {
  ...
  user_data              = base64encode(templatefile("userdata.sh", { db-endpoint = aws_db_instance.db-server.address, user-data-git-token = data.aws_ssm_parameter.example_parameter.value, user-data-git-name = var.git-name }))
  tag_specifications {
